...
already_bought_symbols = [order.symbol for order in orders if order.side == 'buy' and order.status == 'filled']
...
for symbol in symbols:
    ...
    try:
        ...
        print(minute_5_bars)
        print(minute_15_bars)
        print(minute_60_bars)
        ...
        if M_60m_P1 > 0 and M_60m_P1 > M_60m_P2 and M_60m_P2 > M_60m_P3:
            if M_15m_P1 > 0 and H_15m_P1 > limit and H_15m_P2 > limit and H_15m_P3 > limit: 
                if M_5m_C0 > 0 and H_5m_C0 > 0.01 and H_5m_P1 < 0:
                    ...
                    if symbol not in already_bought_symbols:
    ...
    try:
        ...
        print(f"no position {e}")

    try:
        if symbol in already_bought_symbols:
            print(already_bought_symbols)
        ...
        print(f"check if the symbol is sold still in the existing orders or not {e}")

    try:
        if orders.side == 'sell' and orders.status == 'filled':
            already_bought_symbols.remove(symbol)
    except Exception as e:
        print(f"still waiting for sell signal or no position {e}")
