        ...
        for symbol in symbols_5MX0_AB:
            ...
            ## -- EXIT -- ##
                ...
                if symbol not in existing_order_symbols or symbol in buy_order_symbols:
                    ...
                    # Stage 1 (100% position): 60MT, 60MXD, 1DT -> sell 40%
                    # Stage 2 (60% position): 1DXD, 1DT -> sell 30% (of original)
                    # Stage 3 (30% position): 1DX0 -> sell remaining 30%
                        ...
                        # Stage 3: At ~30% entry_quantity (after 1st and 2nd sells)
                        elif 0.20 <= position_pct < 0.40:
                            # Check if 1DX0-line (daily cross down at zero line)
                            if M_1d_P1 < 0.0000 and M_1d_P2 > 0.0000:
                                quantity = int(int(entry_quantity) * 0.30)
                                print(f" - EXIT Stage 3: Taking profit final 30% by 1DX0 on {symbol}.")
                                place_sell_order(symbol, quantity)
                            else:
