...
# It's need 5MX0 in condition which 60M should be like PXE or 60MXU.
# It's need day trade support but there in day trade count limit at 3.
# So, 5MX0 can be modified by adding 60M condition to be 60MX0.
# Surely, it comes with trade off of unconfirmed 60M condition.
# In comparison, if 60M condition is confirmed but entry point could be higher.
# It's clear now that we are unable to make day trade with 5MX0.
# Should we keep 5MX0 with 60m condition as the entry point for swing trade or use only 60m condition?
# If we give more weight to 60m condition and ignore 5MX0 that means more focus on expectation value than perfect entry point, which is never exist.
# Yes, I just checked the 5MX0-60m condition what I found is that there is no perfect entry condition.
# It works in the way of positive expectation value. So, only 60m condition is enough. Then, it works just in the way of probability with market condition.
# This conclusion needs proof.
# I will manage to work on this idea while keep observing if 5MX0 can improve the result or not.
# 31 Aug 2025
        ...
        for symbol in symbols_5MX0_AB:
            ...
            ## -- EXIT -- ##
                ...
                if symbol not in existing_order_symbols or symbol in buy_order_symbols:
                    ...
                    # Strategy 3: Exit by timing (after 5 trading days, excluding weekends/holidays)
                        ...
                        if entry_d in trading_dates:
                            ...
                            # Check if current_date is at least five trading days after entry_d
                            if len(trading_dates) > entry_idx + 5 and current_date >= trading_dates[entry_idx + 5]:
                                print("Strategy 3: Exiting by timing (after 5 trading days)")
                                place_sell_order(symbol, quantity)
                        else:
