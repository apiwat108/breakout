...
if clock.is_open:
    ...
    if EMA_10d_market is not None and C_1d_C0_market is not None and H_1d_C0_market is not None and C_1d_C0_market < EMA_10d_market and H_1d_C0_market < 0.0000:
        ...
        if existing_position_symbols:
            ...
            # Prepare indicators used by exit strategies (bring here from "Market is good to trade")
            try:
                macd_60 = compute_macd(minute_60_bars_open, fast=12, slow=26, signal=9) if 'minute_60_bars_open' in locals() else pd.DataFrame()
            except Exception:
                macd_60 = pd.DataFrame()
            try:
                # Prefer pandas_ta if available, fallback to compute_rsi helper
                if 'minute_60_bars_open' in locals() and hasattr(minute_60_bars_open, 'ta'):
                    rsi_60 = minute_60_bars_open.ta.rsi()
                else:
                    rsi_60 = compute_rsi(minute_60_bars_open)
            except Exception:
                rsi_60 = pd.Series()

            decimal_point = 4
            # Ensure daily MACD and daily close are available
            try:
                M_1d_C0 = round(macd_1['MACD_12_26_9'].iloc[-1], decimal_point) if macd_1 is not None and not macd_1.empty and 'MACD_12_26_9' in macd_1.columns else None
                M_1d_P1 = round(macd_1['MACD_12_26_9'].iloc[-2], decimal_point) if macd_1 is not None and not macd_1.empty and 'MACD_12_26_9' in macd_1.columns else None
                Close_1d_C0 = round(day_1_bars['Close'].iloc[-1], decimal_point) if 'Close' in day_1_bars.columns else None
            except Exception:
                M_1d_C0 = M_1d_P1 = Close_1d_C0 = None

            EMA_10d_bars = day_1_bars['Close'].ewm(span=10, adjust=False).mean()
            EMA_10d = round(EMA_10d_bars.iloc[-1], 2)

            # 60-minute indicators (safe guarded)
            try:
                if macd_60 is not None and not macd_60.empty and 'MACD_12_26_9' in macd_60.columns:
                    ...
                    M_60m_C0 = M_60m_P1 = M_60m_P2 = None

                if macd_60 is not None and not macd_60.empty and 'MACDh_12_26_9' in macd_60.columns:
                    ...
                    H_60m_C0 = H_60m_P1 = H_60m_P2 = H_60m_P3 = None

                RSI_60m_C0 = round(rsi_60.iloc[-1], decimal_point) if hasattr(rsi_60, '__len__') and len(rsi_60) > 0 else None
                ...
                Open_60m_C0 = round(minute_60_bars_open['open'].iloc[-1], decimal_point) if 'open' in minute_60_bars_open.columns else None
                Close_60m_C0 = round(minute_60_bars_open['close'].iloc[-1], decimal_point) if 'close' in minute_60_bars_open.columns else None
            
            except Exception:
                M_60m_C0 = M_60m_P1 = M_60m_P2 = H_60m_C0 = H_60m_P1 = H_60m_P2 = H_60m_P3 = RSI_60m_C0 = Open_60m_C0 = Close_60m_C0 = None
