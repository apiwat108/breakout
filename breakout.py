        ...
        for symbol in symbols_5MX0_AB:
            ...
            ## -- EXIT -- ##
                ...
                if symbol not in existing_order_symbols or symbol in buy_order_symbols:
                    ...
                    # Strategy 6: Taking profit by detecting bearish patterns with valid MACD values
                    elif M_1d_C0 > M_1d_P1 and M_60m_C0 > 0.0000:
                        # Bearish Pin Bar: requires at least one daily bar
                        if day_df_for_patterns is not None and is_bearish_pin_bar(day_df_for_patterns, idx=-1):
                        ...
                        # Bearish Engulfing: requires at least two daily bars (current + previous)
                            ...
                            if is_bearish_engulfing(last2, idx=-1):
                                print(f" - Bearish Engulfing detected for {symbol}.")
                                TRACK_1D_EXIT = f"TRACK_1D_EXIT,{datetime.now().strftime('%Y-%m-%d %H:%M:%S')},{symbol},{market_price},{entry_price},{market_value},{cost},{((market_value - cost) / cost) * 100:.2f}%"
                                print(TRACK_1D_EXIT)
                                place_sell_order(symbol, quantity)
                        
                        # No bearish pattern detected
                        else:
                            print(f" - No bearish pattern detected for {symbol} on daily chart.")
