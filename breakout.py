...
def build_regular_session_60m_bars(minute_5_bars):
    """Build 60-minute bars aligned to 09:30, 10:30, ..., 15:30 ET."""
    if minute_5_bars is None or minute_5_bars.empty:
        return pd.DataFrame()

    minute_5_bars_open = minute_5_bars.copy()
    minute_5_bars_open.columns = [col.lower() for col in minute_5_bars_open.columns]
    required_cols = ['open', 'high', 'low', 'close', 'volume']
    if any(col not in minute_5_bars_open.columns for col in required_cols):
        return pd.DataFrame()

    us_eastern = pytz.timezone('America/New_York')
    if isinstance(minute_5_bars_open.index, pd.MultiIndex):
        minute_5_bars_open.index = minute_5_bars_open.index.get_level_values(-1)
    if minute_5_bars_open.index.tz is None:
        minute_5_bars_open.index = minute_5_bars_open.index.tz_localize('UTC').tz_convert(us_eastern)
    else:
        minute_5_bars_open.index = minute_5_bars_open.index.tz_convert(us_eastern)

    is_market_open_bar = minute_5_bars_open.index.map(
        lambda ts: (ts.hour > 9 or (ts.hour == 9 and ts.minute >= 30)) and ts.hour < 16
    )
    minute_5_bars_open = minute_5_bars_open[is_market_open_bar]
    if minute_5_bars_open.empty:
        return pd.DataFrame()

    try:
        minute_60_bars_open = minute_5_bars_open[required_cols].resample(
            '60min',
            label='left',
            closed='left',
            origin='start_day',
    ...
    try:
        ...
        minute_60_bars_open = minute_60_bars_open[
            minute_60_bars_open.index.map(lambda ts: max_5m_ts >= ts + pd.Timedelta(minutes=55))
        ]
    except Exception:
        pass

    return minute_60_bars_open.dropna(how='any')

def find_60m_bearish_divergence(minute_60_bars, breakout_date):
    """
    Detect 60m bearish divergence after breakout:
    the latest confirmed 60m price peak is higher, while its MACD peak is lower
    than an earlier 60m price/MACD peak after the breakout date.
    """
    try:
        if minute_60_bars is None or len(minute_60_bars) < 6:
            return False, None

        bars = minute_60_bars.copy()
        ...
        if bars.index.tz is None:
            bars.index = bars.index.tz_localize('UTC').tz_convert(us_eastern)
        else:
            bars.index = bars.index.tz_convert(us_eastern)

        if breakout_date is not None:
            bars = bars[bars.index.date >= breakout_date]
        if len(bars) < 6:
            return False, None

        macd_60 = compute_macd(bars, fast=12, slow=26, signal=9)
        if macd_60 is None or macd_60.empty or 'MACD_12_26_9' not in macd_60.columns:
            return False, None

        divergence_df = pd.DataFrame({
            'close': bars['close'],
            'macd': macd_60['MACD_12_26_9'],
        }).dropna()
        if len(divergence_df) < 6:
            return False, None

        current_idx = len(divergence_df) - 2
        current_row = divergence_df.iloc[current_idx]
        current_price_peak = (
            current_row['close'] > divergence_df['close'].iloc[current_idx - 1]
            and current_row['close'] > divergence_df['close'].iloc[current_idx + 1]
        )
        current_macd_peak = (
            current_row['macd'] > divergence_df['macd'].iloc[current_idx - 1]
            and current_row['macd'] > divergence_df['macd'].iloc[current_idx + 1]
        )
        if not (current_price_peak and current_macd_peak):
            return False, None

        for previous_idx in range(1, current_idx - 1):
            previous_row = divergence_df.iloc[previous_idx]
            previous_price_peak = (
                previous_row['close'] > divergence_df['close'].iloc[previous_idx - 1]
                and previous_row['close'] > divergence_df['close'].iloc[previous_idx + 1]
            )
            previous_macd_peak = (
                previous_row['macd'] > divergence_df['macd'].iloc[previous_idx - 1]
                and previous_row['macd'] > divergence_df['macd'].iloc[previous_idx + 1]
            )
            if (
                previous_price_peak
                and previous_macd_peak
                and current_row['close'] > previous_row['close']
                and current_row['macd'] < previous_row['macd']
            ):
                details = {
                    'current_time': divergence_df.index[current_idx],
                    'current_price_peak': float(current_row['close']),
                    'current_macd_peak': float(current_row['macd']),
                    'previous_time': divergence_df.index[previous_idx],
                    'previous_price_peak': float(previous_row['close']),
                    'previous_macd_peak': float(previous_row['macd']),
                }
                return True, details
...
# Function to track manual breakout exit
def track_manual_breakout_exit(symbol, quantity_sold, exit_price, exit_reason):
    """Track partial or full exits for manual breakout entries."""
    try:
        import psycopg2
        ...
        # Find the manual breakout entry for this symbol
        cursor.execute("""
            SELECT id, entry_price, quantity, cost_basis
            FROM manual_breakout_entries
            WHERE symbol = %s
            AND status IN ('filled', 'virtual')
            AND is_active = TRUE
        ...
        if entry:
            ...
            quantity_remaining = original_quantity - quantity_sold
            ...
            cursor.execute("""
                INSERT INTO manual_breakout_partial_exits 
                (entry_id, symbol, exit_date, exit_price, quantity_sold, quantity_remaining, profit_loss_pct, exit_reason)
                VALUES (%s, %s, CURRENT_DATE, %s, %s, %s, %s, %s);
            """, (entry_id, symbol, exit_price, quantity_sold, quantity_remaining, profit_loss_pct, exit_reason))
            
            # If fully exited, update main entry status
            if quantity_remaining <= 0:
                cursor.execute("""
                    UPDATE manual_breakout_entries
                    SET status = 'fully_exited',
                        exit_date = CURRENT_DATE,
                        exit_price = %s,
                        profit_loss_pct = %s,
                        updated_at = CURRENT_TIMESTAMP
                    WHERE id = %s;
                """, (exit_price, profit_loss_pct, entry_id))
                
                print(f"TRACK_MANUAL_EXIT_FULL,{datetime.now().strftime('%Y-%m-%d %H:%M:%S')},{symbol},{exit_price},{profit_loss_pct:.2f}%")
            else:
                print(f"TRACK_MANUAL_EXIT_PARTIAL,{datetime.now().strftime('%Y-%m-%d %H:%M:%S')},{symbol},{exit_price},{quantity_sold}/{original_quantity},{profit_loss_pct:.2f}%")
        ...
        print(f"⚠️  Error tracking manual breakout exit: {e}")

# Function to place sell order
def place_sell_order(symbol, quantity):
    try:
        # Get current market price for tracking
        current_price = None
        try:
            minute_5_bars_temp = get_alpaca_bars(symbol, TimeFrame(5, TimeFrameUnit.Minute), start=datetime.now() - timedelta(days=1))
            if not minute_5_bars_temp.empty and 'Close' in minute_5_bars_temp.columns:
                minute_5_bars_temp.columns = [col.lower() for col in minute_5_bars_temp.columns]
                current_price = round(minute_5_bars_temp['close'].iloc[-1], 2)
        except:
            pass
...
if clock.is_open:
    ...
    market_indicators_available = M_1d_C0_market is not None and H_1d_C0_market is not None
    market_is_not_good_to_trade = (
        market_indicators_available
        and (
            (M_1d_C0_market < 0.0000 and H_1d_C0_market < 0.0000) 
            or (C_1d_C0_market_P1 < EMA_10d_market and H_1d_C0_market < 0.0000)
        )
    )
    market_is_good_to_trade = (
        market_indicators_available
        and (
            M_1d_C0_market > 0.0000
            or (M_1d_C0_market < 0.0000 and H_1d_C0_market > 0.0000)
        )
    )
    if market_is_not_good_to_trade:
        print(" - Market is not good to trade. No new manual breakout entries will be opened.")
    elif market_is_good_to_trade:
        print(" - Market is good to trade. Manual breakout entries are allowed.")
    else:
        print(" - Market condition is not defined. No new manual breakout entries will be opened.")

    # ========== MANUAL BREAKOUT ENTRY EXECUTION ==========
    # Check for pending manual breakout entries from config file (no database needed)
    # New entries run only when the SPY market condition is good.
    try:
        import config_manual
        
        if not market_is_good_to_trade:
            if hasattr(config_manual, 'MANUAL_BREAKOUT_ENTRIES') and config_manual.MANUAL_BREAKOUT_ENTRIES:
                print(f"\n{'='*70}")
                print("📊 SKIPPING MANUAL BREAKOUT ENTRIES")
                print(f"{'='*70}")
                print("Market condition does not allow new entries.")
                print("Existing manual breakout positions will still be checked for exits.")
                print(f"{'='*70}\n")
        elif hasattr(config_manual, 'MANUAL_BREAKOUT_ENTRIES') and config_manual.MANUAL_BREAKOUT_ENTRIES:
            pending_entries = config_manual.MANUAL_BREAKOUT_ENTRIES
            
            print(f"\n{'='*70}")
            print(f"📊 EXECUTING {len(pending_entries)} MANUAL BREAKOUT ENTRIES")
            print(f"{'='*70}")
            
            # Get current date as date object for comparisons (and string for API calls)
            ...
            # Check existing orders (to prevent duplicate orders today)
            orders = api.list_orders(status='all', after=current_date_str)
            ...
            use_buying_power_for_risk = getattr(config_manual, 'USE_BUYING_POWER_FOR_RISK', False)
            buying_power_usage_limit = getattr(config_manual, 'BUYING_POWER_USAGE_LIMIT', 1.0)
            usable_buying_power = buying_power * buying_power_usage_limit
            pending_buying_power_usage = 0.0
            
            # Track which entries were executed (to remove from list later)
            executed_entry_ids = []
            
            for entry in pending_entries:
                symbol = entry['symbol']
                stop_loss_price = float(entry['stop_loss'])
                entry_id = entry.get('entry_id', 0)
                breakout_date_str = entry.get('breakout_date', 'N/A')
                
                print(f"\n{symbol} (Manual Entry #{entry_id})")
                print(f"  Breakout Date: {breakout_date_str}")
                print(f"  Stop Loss: ${stop_loss_price:.2f}")
                
                # Check if already have position
                if symbol in existing_position_symbols:
                    print(f"  ⚠️  Already in position for {symbol}, skipping entry")
                    continue
                
                # Check if already placed order today
                if symbol in existing_order_symbols:
                    print(f"  ⚠️  Order already placed for {symbol} today, skipping entry")
                    continue
                
                try:
                    # Check if entry is expired (more than 1 trading day past breakout date)
                    try:
                        breakout_date = datetime.strptime(breakout_date_str, '%Y-%m-%d').date()
                        
                        # Get trading calendar
                        calendar = api.get_calendar(start=breakout_date.strftime('%Y-%m-%d'), end=current_date.strftime('%Y-%m-%d'))
                        trading_dates = [day.date.date() if hasattr(day.date, 'date') else day.date for day in calendar]
                        
                        # Check if we're past the entry window (next trading day after breakout)
                        if breakout_date in trading_dates:
                            breakout_idx = trading_dates.index(breakout_date)
                            # Entry window is the next trading day after breakout (index + 1)
                            if len(trading_dates) > breakout_idx + 1:
                                entry_window_date = trading_dates[breakout_idx + 1]
                                
                                if current_date > entry_window_date:
                                    print(f"  ⏰ ENTRY EXPIRED: Entry window was {entry_window_date}, today is {current_date}")
                                    print(f"  ✓ Tracking as EXPIRED entry (logged to file, keeping in config for monitoring)")
                                    ...
                                    print(f"TRACK_MANUAL_EXPIRED,{datetime.now().strftime('%Y-%m-%d %H:%M:%S')},{symbol},{current_price},{stop_loss_price},{breakout_date_str},{entry_window_date},entry_window_passed")
                                    # Don't add to executed_entry_ids - keep in config for exit monitoring
                                    continue
                    except ValueError:
                        print(f"  ⚠️  Invalid breakout date format: {breakout_date_str}")
                    
                    # Get current market price
                    minute_5_bars_temp = get_alpaca_bars(symbol, TimeFrame(5, TimeFrameUnit.Minute), start=datetime.now() - timedelta(days=1))
                    if minute_5_bars_temp.empty or 'Close' not in minute_5_bars_temp.columns:
                    ...
                    # Entry should only happen during market open hour or on the first run of the day
                    ny_now = datetime.now(pytz.timezone('America/New_York'))
                    market_open_hour = getattr(config_manual, 'ENTRY_WINDOW_HOUR', 9)
                    first_script_run_today = len(existing_order_symbols) == 0
                    entry_window_ok = (ny_now.hour == market_open_hour) or first_script_run_today
                    
                    print(f"  Current Price: ${current_price:.2f}")
                    print(f"  Current Time (ET): {ny_now.strftime('%H:%M:%S')}")
                    print(f"  Entry window hour: {market_open_hour}:00")
                    print(f"  First run today: {first_script_run_today}")
                    
                    if not entry_window_ok:
                        print(f"  ⏰ Skipping entry because it is outside the allowed entry window hour and not the first script run today.")
                        continue
                    
                    # Calculate risk-based position size
                    risk_base_value = usable_buying_power if use_buying_power_for_risk else portfolio_value
                    risk_base_label = "usable buying power" if use_buying_power_for_risk else "portfolio value"
                    risk_dollars = risk_base_value * config_manual.RISK_PERCENTAGE
                    price_diff = current_price - stop_loss_price
                    
                    if price_diff <= 0:
                        print(f"  ❌ Invalid: Current price (${current_price:.2f}) <= Stop loss (${stop_loss_price:.2f})")
                        continue
                    
                    # Formula: Position Size = (Risk Base × Risk%) / (Entry Price - Stop Loss)
                    risk_pct_per_share = price_diff / current_price
                    position_value = risk_dollars / risk_pct_per_share
                    
                    # Apply position value limits
                    if position_value > config_manual.MAX_POSITION_VALUE:
                        position_value = config_manual.MAX_POSITION_VALUE
                    elif position_value < config_manual.MIN_POSITION_VALUE:
                        position_value = config_manual.MIN_POSITION_VALUE
                    
                    quantity = int(position_value / current_price)
                    actual_position_value = quantity * current_price
                    actual_risk_pct = (price_diff / current_price) * 100
                    remaining_usable_buying_power = max(0.0, usable_buying_power - pending_buying_power_usage)
                    
                    print(f"  Portfolio Value: ${portfolio_value:,.2f}")
                    print(f"  Buying Power: ${buying_power:,.2f}")
                    print(f"  Usable Buying Power ({buying_power_usage_limit:.0%}): ${remaining_usable_buying_power:,.2f}")
                    print(f"  Risk ({config_manual.RISK_PERCENTAGE:.0%} of {risk_base_label}): ${risk_dollars:,.2f}")
                    print(f"  Risk per share: ${price_diff:.2f} ({actual_risk_pct:.2f}%)")
                    print(f"  Calculated Quantity: {quantity} shares")
                    print(f"  Position Value: ${actual_position_value:,.2f}")
                    
                    # Check if sufficient buying power remains after the configured buffer
                    if actual_position_value > remaining_usable_buying_power:
                        print(f"  ⚠️  Insufficient usable buying power (need ${actual_position_value:,.2f}, have ${remaining_usable_buying_power:,.2f})")
                        print(f"  ✓ Tracking as VIRTUAL entry (logged to file)")
                        print(f"TRACK_MANUAL_VIRTUAL,{datetime.now().strftime('%Y-%m-%d %H:%M:%S')},{symbol},{current_price},{stop_loss_price},{quantity},{actual_position_value},insufficient_buying_power")
                        executed_entry_ids.append(entry_id)
                    else:
                        ...
                        try:
                            ...
                            print(f"  ✅ BUY ORDER PLACED: {quantity} shares @ market")
                            pending_buying_power_usage += actual_position_value
                            executed_entry_ids.append(entry_id)
                            print(f"TRACK_MANUAL_ENTRY,{datetime.now().strftime('%Y-%m-%d %H:%M:%S')},{symbol},{current_price},{stop_loss_price},{quantity},{actual_position_value}")
            ...
            print(f"\n{'='*70}")
            print(f"✓ Manual breakout execution complete")
            if executed_entry_ids:
                print(f"⚠️  IMPORTANT: Remove executed entries from config_manual.py")
                print(f"   Entry IDs executed: {executed_entry_ids}")
            print(f"{'='*70}\n")
        
    except Exception as e:
        print(f"⚠️  Manual breakout execution error: {e}")
        print("Continuing with regular breakout strategy...\n")
    # ========== END MANUAL BREAKOUT ENTRY EXECUTION ==========

    # ========== MANUAL BREAKOUT EXIT LOGIC ==========
    # Check if any positions are from manual breakout entries and apply exit strategies
    # This runs independently of market conditions
    try:
        import config_manual
        ...
        if hasattr(config_manual, 'MANUAL_BREAKOUT_ENTRIES') and config_manual.MANUAL_BREAKOUT_ENTRIES:
            ...
            if existing_position_symbols:
                # Create a dict for quick lookup of manual entries by symbol
                manual_entries_dict = {entry['symbol']: entry for entry in config_manual.MANUAL_BREAKOUT_ENTRIES}
                
                print(f"\n{'='*70}")
                print(f"🎯 CHECKING MANUAL BREAKOUT EXITS")
                print(f"{'='*70}")
                
                for symbol in existing_position_symbols:
                    # Check if this position is from a manual breakout entry
                    if symbol not in manual_entries_dict:
                        continue
                    
                    manual_entry = manual_entries_dict[symbol]
                    stop_loss_price = float(manual_entry['stop_loss'])
                    breakout_date_str = manual_entry.get('breakout_date', 'N/A')
                    
                    print(f"\n{'='*70}")
                    print(f"🎯 MANUAL BREAKOUT EXIT CHECK: {symbol}")
                    print(f"   Breakout Date: {breakout_date_str}")
                    print(f"   Stop Loss: ${stop_loss_price:.2f}")
                    print(f"{'='*70}")
                    ...
                    try:
                        ...
                        minute_5_bars_temp = get_alpaca_bars(symbol, TimeFrame(5, TimeFrameUnit.Minute), start=datetime.now() - timedelta(days=1))
                        day_1_bars_temp = get_alpaca_bars(symbol, TimeFrame.Day, start=datetime.now() - timedelta(days=180))
                        
                        if minute_5_bars_temp.empty or day_1_bars_temp.empty:
                            print(f"  ⚠️  Could not fetch price data for {symbol}")
                            continue
                        
                        minute_5_bars_temp.columns = [col.lower() for col in minute_5_bars_temp.columns]
                        day_1_bars_temp.columns = [col.lower() for col in day_1_bars_temp.columns]
                        ...
                        print(f"  Current P/L: {((market_value - cost) / cost) * 100:.2f}%")
                        print(f"  Position: {quantity}/{entry_quantity} shares")
                        
                        # Calculate MACD for profit-taking stages
                        macd_1_temp = compute_macd(day_1_bars_temp)
                        M_1d_C0 = round(macd_1_temp['MACD_12_26_9'].iloc[-1], 4) if 'MACD_12_26_9' in macd_1_temp.columns else None
                        M_1d_P1 = round(macd_1_temp['MACD_12_26_9'].iloc[-2], 4) if 'MACD_12_26_9' in macd_1_temp.columns else None
                        M_1d_P2 = round(macd_1_temp['MACD_12_26_9'].iloc[-3], 4) if 'MACD_12_26_9' in macd_1_temp.columns else None
                        M_1d_P3 = round(macd_1_temp['MACD_12_26_9'].iloc[-4], 4) if 'MACD_12_26_9' in macd_1_temp.columns else None
                        H_1d_P1 = round(macd_1_temp['MACDh_12_26_9'].iloc[-2], 4) if 'MACDh_12_26_9' in macd_1_temp.columns else None
                        H_1d_P2 = round(macd_1_temp['MACDh_12_26_9'].iloc[-3], 4) if 'MACDh_12_26_9' in macd_1_temp.columns else None

                        minute_60_bars_temp = build_regular_session_60m_bars(
                            get_alpaca_bars(
                                symbol,
                                TimeFrame(5, TimeFrameUnit.Minute),
                                start=datetime.combine(entry_d, datetime.min.time()),
                            )
                        )
                        has_60m_bearish_divergence, bearish_divergence_details = find_60m_bearish_divergence(
                            minute_60_bars_temp,
                            breakout_date=entry_d,
                        )
                        
                        # EXIT STRATEGY 1: Stop Loss Hit
                        if current_price <= stop_loss_price:
                            # Check if actual loss exceeds 2% of portfolio
                            account = api.get_account()
                            portfolio_value = float(account.portfolio_value)
                            actual_loss_dollars = cost - market_value
                            actual_loss_pct_of_portfolio = (actual_loss_dollars / portfolio_value) * 100
                            risk_threshold = 2.0
                            
                            print(f"  ❌ STOP LOSS HIT: ${current_price:.2f} <= ${stop_loss_price:.2f}")
                            print(f"  💰 Actual Loss: ${actual_loss_dollars:.2f} ({actual_loss_pct_of_portfolio:.2f}% of portfolio)")
                            print(f"  📊 Risk Threshold: {risk_threshold}% of portfolio")
                            
                            if actual_loss_pct_of_portfolio >= risk_threshold:
                                print(f"  📤 Selling 100% position ({quantity} shares) - Loss exceeds {risk_threshold}% risk")
                                order_success = place_sell_order(symbol, quantity)
                                if order_success:
                                    print(f"TRACK_MANUAL_EXIT_STOP_LOSS,{datetime.now().strftime('%Y-%m-%d %H:%M:%S')},{symbol},{current_price},{entry_price},{stop_loss_price},{quantity},{market_value},{cost},{((market_value - cost) / cost) * 100:.2f}%")
                                else:
                                    print(f"  ⚠️  No TRACK_MANUAL_EXIT_STOP_LOSS logged (order failed)")
                            else:
                                print(f"  💎 Holding: Loss ({actual_loss_pct_of_portfolio:.2f}%) below {risk_threshold}% threshold")
                        
                        # EXIT STRATEGY 2: Taking Profit (Staged Partial Exits)
                        elif market_value >= (1.0000 * cost) and trading_day != entry_d:
                            position_pct = (quantity / entry_quantity) if entry_quantity > 0 else 0
                            
                            # Stage 1: At 100% entry_quantity - sell 20% on 1DT
                            if position_pct >= 0.95:
                                if (
                                    M_1d_P1 is not None
                                    and M_1d_P2 is not None
                                    and M_1d_P3 is not None
                                    and M_1d_P1 < M_1d_P2
                                    and M_1d_P2 > M_1d_P3
                                    and M_1d_P1 > 0.0000
                                ):
                                    quantity_to_sell = int(int(entry_quantity) * 0.20)
                                    print(f"  ✅ EXIT Stage 1: Taking profit 20% by 1DT on {symbol}.")
                                    print(f"  📤 Selling {quantity_to_sell} shares (20% of {entry_quantity})")
                                    order_success = place_sell_order(symbol, quantity_to_sell)
                                    if order_success:
                                        print(f"TRACK_MANUAL_EXIT_STAGE1,{datetime.now().strftime('%Y-%m-%d %H:%M:%S')},{symbol},{current_price},{entry_price},{quantity_to_sell},{market_value},{cost},{((market_value - cost) / cost) * 100:.2f}%")
                                    else:
                                        print(f"  ⚠️  No TRACK_MANUAL_EXIT_STAGE1 logged (order failed)")
                                elif has_60m_bearish_divergence:
                                    quantity_to_sell = int(int(entry_quantity) * 0.20)
                                    print(f"  ✅ EXIT Stage 1: Taking profit 20% by 60M Bearish Divergence on {symbol}.")
                                    if bearish_divergence_details:
                                        print(
                                            "  📉 60M divergence: "
                                            f"price {bearish_divergence_details['previous_price_peak']:.2f} -> "
                                            f"{bearish_divergence_details['current_price_peak']:.2f}, "
                                            f"MACD {bearish_divergence_details['previous_macd_peak']:.4f} -> "
                                            f"{bearish_divergence_details['current_macd_peak']:.4f}"
                                        )
                                    print(f"  📤 Selling {quantity_to_sell} shares (20% of {entry_quantity})")
                                    order_success = place_sell_order(symbol, quantity_to_sell)
                                    if order_success:
                                        print(f"TRACK_MANUAL_EXIT_STAGE1_60M_BEAR_DIV,{datetime.now().strftime('%Y-%m-%d %H:%M:%S')},{symbol},{current_price},{entry_price},{quantity_to_sell},{market_value},{cost},{((market_value - cost) / cost) * 100:.2f}%")
                                    else:
                                        print(f"  ⚠️  No TRACK_MANUAL_EXIT_STAGE1_60M_BEAR_DIV logged (order failed)")
                                else:
                                    print(f"  💎 Let the stock running for the 1st volume-position exit on {symbol}.")
                            
                            # Stage 2: At ~80% entry_quantity - sell 30% on 1DXD
                            elif 0.70 <= position_pct < 0.90:
                                if H_1d_P1 is not None and H_1d_P2 is not None:
                                    if H_1d_P1 < 0.0000 and H_1d_P2 > 0.0000:
                                        quantity_to_sell = int(int(entry_quantity) * 0.3)
                                        print(f"  ✅ EXIT Stage 2: Taking profit 30% by 1DXD on {symbol}.")
                                        print(f"  📤 Selling {quantity_to_sell} shares (30% of {entry_quantity})")
                                        order_success = place_sell_order(symbol, quantity_to_sell)
                                        if order_success:
                                            print(f"TRACK_MANUAL_EXIT_STAGE2,{datetime.now().strftime('%Y-%m-%d %H:%M:%S')},{symbol},{current_price},{entry_price},{quantity_to_sell},{market_value},{cost},{((market_value - cost) / cost) * 100:.2f}%")
                                        else:
                                            print(f"  ⚠️  No TRACK_MANUAL_EXIT_STAGE2 logged (order failed)")
                                    else:
                                        print(f"  💎 Let the stock running for the 2nd volume-position exit on {symbol}.")
                            
                            # Stage 3: At ~50% entry_quantity - sell 50% on 1DX0
                            elif 0.40 <= position_pct < 0.60:
                                if M_1d_P1 is not None and M_1d_P2 is not None and M_1d_C0 is not None:
                                    if M_1d_P1 < 0.0000 and M_1d_P2 > 0.0000 and M_1d_C0 < M_1d_P1:
                                        quantity_to_sell = int(int(entry_quantity) * 0.50)
                                        print(f"  ✅ EXIT Stage 3: Taking profit final 50% by 1DX0 on {symbol}.")
                                        print(f"  📤 Selling {quantity_to_sell} shares (50% of {entry_quantity})")
                                        order_success = place_sell_order(symbol, quantity_to_sell)
                                        if order_success:
                                            print(f"TRACK_MANUAL_EXIT_STAGE3,{datetime.now().strftime('%Y-%m-%d %H:%M:%S')},{symbol},{current_price},{entry_price},{quantity_to_sell},{market_value},{cost},{((market_value - cost) / cost) * 100:.2f}%")
                                        else:
                                            print(f"  ⚠️  No TRACK_MANUAL_EXIT_STAGE3 logged (order failed)")
                                    else:
                                        print(f"  💎 Let the stock running for the 3rd volume-position exit on {symbol}.")
                            
                            else:
                                print(f"  💎 Position at {position_pct*100:.1f}% of entry, outside staged exit ranges.")
                        
                        # EXIT STRATEGY 3: Timing Limit (30 trading days, no profit)
                        elif market_value <= (1.0000 * cost):
                            ...
                            if entry_d in trading_dates:
                                ...
                                # Check if current_date is at least 30 trading days after entry_d
                                if len(trading_dates) > entry_idx + 30 and current_date >= trading_dates[entry_idx + 30]:
                                    print(f"  ⏰ TIMING LIMIT: Over 30 trading days with no profit")
                                    print(f"  📤 Selling 100% position ({quantity} shares)")
                                    order_success = place_sell_order(symbol, quantity)
                                    if order_success:
                                        print(f"TRACK_MANUAL_EXIT_TIMING,{datetime.now().strftime('%Y-%m-%d %H:%M:%S')},{symbol},{current_price},{entry_price},{quantity},{market_value},{cost},{((market_value - cost) / cost) * 100:.2f}%")
                                    else:
                                        print(f"  ⚠️  No TRACK_MANUAL_EXIT_TIMING logged (order failed)")
                                else:
                                    print(f"  ⏳ Holding: {len(trading_dates) - entry_idx - 1} trading days since entry")
                            else:
                                print(f"  ⚠️  Entry date {entry_d} not found in trading calendar")
                        
                        else:
                            print(f"  ✓ No manual exit conditions met")
                    
                    except Exception as e:
                        print(f"  ❌ Error processing manual exit for {symbol}: {e}")
                    
                    print(f"{'='*70}\n")
                
                print(f"{'='*70}")
                print(f"✓ Manual breakout exit check complete")
                print(f"{'='*70}\n")
    
    except Exception as e:
        print(f"⚠️  Manual breakout exit check error: {e}\n")
    # ========== END MANUAL BREAKOUT EXIT LOGIC ==========

    # ========== OLD 60MXU STRATEGY - COMMENTED OUT ==========
    # The following sections are commented out because we're now using manual breakout strategy only
    # If you want to re-enable 60MXU automated entry/exit, uncomment these sections
    
    # Market is not good to trade
    if False and EMA_10d_market is not None and C_1d_C0_market is not None and H_1d_C0_market is not None and (M_1d_C0_market < 0.0000 and H_1d_C0_market < 0.0000):
        # This section is commented out - using manual breakout strategy only
        ...
        if existing_position_symbols:
            ...
            for symbol in existing_position_symbols:
                ...
                # ========== OLD 60MXU EXIT STRATEGIES - COMMENTED OUT ==========
                # These are commented out to prevent interference with manual breakout exits
                # Only manual breakout exit logic (below) should be active
                ...
                if False and M_60m_C0 < 0.0000 and entry_d is not None:
                    # Get all trading dates between entry_d and current_date
                ...
                if False and M_1d_C0 > M_1d_P1 and M_60m_C0 > 0.0000:
                    try:
                        if day_df_for_patterns is None or day_df_for_patterns.empty:
                            ...
                            try:
                                ...
                                if isinstance(last_index_val, pd.Timestamp):
                                    ...
                                    last_bar_ts = pd.to_datetime(last_index_val, errors='coerce', utc=True)
                            ...
                            if entry_d is not None and prev_day_df is not None and not prev_day_df.empty:
                                try:
                                    ...
                                    prev_day_ts = pd.to_datetime(prev_idx_val, utc=True)
        ...
        # ========== MANUAL BREAKOUT EXIT LOGIC (Market Not Good) ==========
        # Check if any positions are from manual breakout entries and apply exit strategies
        try:
            import config_manual
                ...
                for symbol in existing_position_symbols:
                    ...
                    try:
                        ...
                        # EXIT STRATEGY 2: Taking Profit (Staged Partial Exits)
                            ...
                            # Stage 1: At 100% entry_quantity - sell 20% on 1DT
                            if position_pct >= 0.95:
                                if M_1d_P1 is not None and M_1d_P2 is not None and M_1d_P3 is not None:
                        ...
                        # EXIT STRATEGY 3: Timing Limit (5 trading days, no profit)
                        elif market_value <= (1.0000 * cost):
                            # Get all trading dates between entry_d and current_date
                            calendar = api.get_calendar(start=entry_d.strftime('%Y-%m-%d'), end=current_date.strftime('%Y-%m-%d'))
                            trading_dates = [day.date.date() if hasattr(day.date, 'date') else day.date for day in calendar]
                            ...
                            if entry_d in trading_dates:
                                entry_idx = trading_dates.index(entry_d)
                                # Check if current_date is at least 5 trading days after entry_d
                                if len(trading_dates) > entry_idx + 5 and current_date >= trading_dates[entry_idx + 5]:
                                    print(f"  ⏰ TIMING LIMIT: Over 5 trading days with no profit")
                                    print(f"  📤 Selling 100% position ({quantity} shares)")
                                    print(f"TRACK_MANUAL_EXIT_TIMING,{datetime.now().strftime('%Y-%m-%d %H:%M:%S')},{symbol},{current_price},{entry_price},{quantity},{market_value},{cost},{((market_value - cost) / cost) * 100:.2f}%")
                                    place_sell_order(symbol, quantity)
                                else:
    ...
    # Market is good to trade
    elif False and (M_1d_C0_market > 0.0000 or (M_1d_C0_market < 0.0000 and H_1d_C0_market > 0.0000)):
        # This section is commented out - using manual breakout strategy only
        print(f"\nMarket is good to trade.")
        ...
        for symbol in symbols_5MX0_AB:
            ...
            if symbol not in existing_position_symbols:
                ...
                if symbol not in existing_order_symbols:
                    ...
                    # Entry After Breakout Date
                    # In put with new format
                    # symbol, stop_loss_price, take_profit_price, entry_price, entry_time, exit_time, profit_loss_percentage
            ...
            ## -- EXIT -- ##
                ...
                if symbol not in existing_order_symbols or symbol in buy_order_symbols:
                    ...
                    if False and market_value >= (1.0000 * cost) and trading_day != entry_d:
        ...
        # ========== MANUAL BREAKOUT EXIT LOGIC (Market Good) ==========
        # Check if any positions are from manual breakout entries and apply exit strategies
        try:
            import config_manual
