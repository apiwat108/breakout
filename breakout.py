...
TRAILING_STOP_PERCENTAGE = 0.05  # 5.0%
TRAILING_STOP_PERCENTAGE_LIMIT = 0.08  # 5.0%
        ...
        if symbol not in existing_position_symbols:
            ...
            if symbol not in existing_order_symbols:
                ...
                # Entry Position 0: 60MX0 on M_1d_C0 < 0.0000 for Breakout-6
                    ...
                    print(f" - It's buy signal for Entry Position 0: on M_1d_C0 < 0.0000")
                ...
                elif M_60m_C0 > 0.0000 and H_60m_C0 > 0.0000 and H_60m_P1 < 0.0000 and H_1d_C0 > 0.0000:
            ...
            try:
                ...
                entry_date = entry_date.astimezone(pytz.timezone('America/New_York')).date()
                ...
                # Calculate trailing stop price limit to 8% 
                trailing_stop_price_limit = highest_price * (1 - TRAILING_STOP_PERCENTAGE_LIMIT)
                ...
                if market_price <= trailing_stop_price and M_60m_C0 < 0.0000 and market_value > (1.0075 * cost):
                    print(f" - Trailing stop hit. Placing sell order for {symbol} at {market_price}.\n")
                    place_sell_order(symbol, quantity)           

                # VERI 11 Oct 2024 -35% but M_60m_C0 > 0.0000
                elif market_price <= trailing_stop_price_limit and market_value > (1.0075 * cost):
                    print(f" - Trailing stop limit hit. Placing sell order for {symbol} at {market_price}.\n")
                    place_sell_order(symbol, quantity)   

            except Exception as e:
                print(f" - The error as {e}")

            # Need work out on exit strategy

            # Strategy 1 for taking profit whatever over 50.0%            
            if market_value > (1.50 * cost):
                # VRAX 29 Aug 2024 turn 20% profit down to 5% cut loss (not updated)
                ...
                print(f" - It's taking-profit signal with whatever over 5.0% profit.")
                place_sell_order(symbol, quantity)

            # Strategy 2 for taking profit at least 0.75%
            # elif market_value > (1.0075 * cost):
            #     if current_date != entry_date:
            #         print(f" - It's taking-profit signal with min of 0.75% profit.")
            #         place_sell_order(symbol, quantity)
            #     else:
            #         print(f" - Due to day-trade limit, avoid sell {symbol} for taking profit on the same day as entry date ({entry_date})\n")

            # Strategy 3 for cutting loss should not greater than 5%
            elif market_value < (0.95 * cost) and M_60m_C0 < 0.0000 and H_60m_C0 < 0.0000:
                # LAAC 24 Oct 2024 Swing lower than -5% at -6.9% then return to profit 4%
                # Swing low with M_60m_C0 > 0.0000 just H_60m_C0 < 0.0000 as well as H_1d_C0 > 0.0000
                # Try to adjust to relative signal cut loss instead of absolute cut-loss percentage
                print(f" - It's cut-loss signal with Strategy 3 (> -5% and M_60m_C0 < 0.0000).")
                place_sell_order(symbol, quantity)

            # Strategy 4 F60MX0 for false 60MX0 and allow 3% for variation
