        ...
        for symbol in symbols_5MX0_AB:
            ...
            ## -- EXIT -- ##
                ...
                if symbol not in existing_order_symbols or symbol in buy_order_symbols:
                    ...
                    # Strategy 6: Taking profit by detecting bearish patterns on the previous FULLY-CLOSED daily bar
                    # Use the previous trading day's single 1-day candle (e.g., if running at 09:30 on Oct 8, check Oct 7)
                        ...
                        try:
                            if day_df_for_patterns is None or day_df_for_patterns.empty:
                                ...
                                try:
                                    last_bar_ts = pd.to_datetime(day_df_for_patterns.index[-1])
                                    ...
                                    if getattr(last_bar_ts, 'date', lambda: last_bar_ts)() >= ny_today:
                                        prev_idx = -2
                                    else:
                                        prev_idx = -1
                                except Exception:
                                    # Fallback: if we can't parse dates, prefer the penultimate bar when available
                                ...
                                if prev_day_df is not None and not prev_day_df.empty and is_bearish_pin_bar(prev_day_df, idx=-1):
