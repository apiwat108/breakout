...
if clock.is_open:
    ...
    if EMA_10d_market is not None and C_1d_C0_market is not None and H_1d_C0_market is not None and C_1d_C0_market < EMA_10d_market and H_1d_C0_market < 0.0000:
        print(f"\nSPY is below EMA_10 ({C_1d_C0_market} < {EMA_10d_market}) and H_1d_C0_market ({H_1d_C0_market}) < 0.0000.")
    ...
    elif C_1d_C0_market > EMA_10d_market and C_1d_C0_market_P1 > EMA_10d_market_P1 and ((H_1d_C0_market > 0.0000) or (H_1d_C0_market < 0.0000 and M_1d_C0_market > 0.0000)):
        print(f"\nMarket is good to trade.")
        print(f"SPY is above EMA_10 ({C_1d_C0_market} > {EMA_10d_market}) and ({C_1d_C0_market_P1} > {EMA_10d_market_P1}) and H_1d_C0_market ({H_1d_C0_market}) > 0.0000.")
                ...
                if symbol not in existing_order_symbols or symbol in buy_order_symbols:
                    ...
                    # Strategy 5: Exit by timing (after 3 trading days, excluding weekends/holidays)
                        ...
                        if entry_d in trading_dates:
                            ...
                            # Check if current_date is at least three trading days after entry_d
                            if len(trading_dates) > entry_idx + 3 and current_date >= trading_dates[entry_idx + 3]:
                                print("Strategy 5: Exiting by timing (after 3 trading days)")
                                place_sell_order(symbol, quantity)
                        else:
                            print(f"Entry date {entry_d} not found in trading calendar, skipping timing exit.")
